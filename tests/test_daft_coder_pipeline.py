...
# Note: Source data has no nulls, so prompts instruct the agent to
# first introduce nulls via a condition, then apply a fill strategy.
# ==========================================
...
# signup_date is stored as a DD/MM/YYYY string
# ==========================================
...
# ==========================================
# ACTIVE TEST SUITE — swap this to run a different category
# ==========================================
TEST_SUITE = DATA_MASKING_TEST_SUITE[0:15]  # <--- CHANGE THIS TO RUN A DIFFERENT TEST SUITE
...
class LLMBehaviorFilter(logging.Filter):
    """Allows only critical LLM process logs, suppressing raw data and RAG dumps."""
    def filter(self, record):
        msg = record.getMessage()
        
        # 1. Block massive text dumps
        if "--- CODER STATE ---" in msg or "=== RETRIEVED DAFT DOCUMENTATION ===" in msg:
            return False
            
        # 2. Block progress bars and embedding model noise
        if "Batches:" in msg or "Loading weights:" in msg or "HTTP Request" in msg:
            return False
            
        # 3. Block SQL and Ray noise (unless it's an explicit error)
        if "Using sqlalchemy" in msg or "PhysicalScan->" in msg or "[ray_job_runner]" in msg:
            if record.levelno < logging.ERROR:
                return False
                
        # 4. Block massive schema/dataframe prints (optional, adjust as needed)
...
def setup_logging():
    ...
    # 1. Mute noisy 3rd-party loggers completely
    noisy_loggers = [
    ...
    # 2. Setup standard Python logging to file
    ...
    # Apply our custom filter to the file handler
    file_handler.addFilter(LLMBehaviorFilter())
    root_logger.addHandler(file_handler)
    
    # 3. Redirect terminal output to capture print() and crash traces
...
def get_pg_connection(creds):
    """Helper to create a raw psycopg2 connection."""
...
def run_single_transfer(test_name, prompt, source_creds, dest_creds, verify_fn):
    ...
    # 0. Pre-run cleanup: Truncate destination table to ensure clean deterministic test
    try:
        conn = get_pg_connection(dest_creds)
        conn.autocommit = True
        with conn.cursor() as cur:
            cur.execute(f"TRUNCATE TABLE {dest_creds['table']};")
        conn.close()
        print("🧹 Truncated destination table for a clean test run.")
    except Exception as e:
        print(f"⚠️ Could not truncate destination table: {e}")

    print("⚙️ Running agent pipeline to generate injection script (this may take a moment)...")
    
    # Using 'postgres' for both source and destination
    ...
    # ==========================================
    if any([coder_def.get('syntax_error'), coder_def.get('static_semantic_error'), 
            coder_def.get('execution_error'), coder_def.get('logical_semantic_error')]):
        print(f"\n❌ [{test_name}] Code generation pipeline failed with internal errors.")
        return "FAIL (Code Gen)"
        ...
        job_id = f"bench-{test_name}-{uuid.uuid4().hex[:6]}"
        ...
        # ==========================================
        print(f"\n🔍 Executing deterministic validation against {dest_creds['table']}...")
        try:
            conn = get_pg_connection(dest_creds)
            # Load the destination table directly into pandas via psycopg2
            df = pd.read_sql(f"SELECT * FROM {dest_creds['table']}", conn)
            conn.close()
            ...
            if verify_fn(df):
                ...
                print(f"❌ [{test_name}] VERIFICATION FAILED: Pipeline ran but logic produced incorrect output.")
        ...
        print(f'\n⚠️ [{test_name}] Pipeline completed, but no script was returned.')
        return "FAIL (Unknown)"
...
def run_sequential_tests():
    ...
    # Pointing to the Postgres database for the source
    source_creds = {
        "host": "10.111.16.4",
        "port": 5432,
        "user": "avaloka",
    ...
    # Base destination credentials for Postgres
    base_dest_creds = {
        "host": "10.111.16.4",
        "port": 5432,
        "user": "avaloka",
    ...
    print("\n" + "="*60)
    print(" 📊 CLOUD POSTGRES BENCHMARK REPORT")
    print("="*60)
    
    report_df = pd.DataFrame(results)
    passed = len(report_df[report_df['status'] == 'PASS'])
    
    print(f"Total Tests: {len(report_df)} | Passed: {passed}")
