...
import { ChevronDown, CheckCircle2, X, HelpCircle, PlusCircle, Check } from "lucide-react";
...
const emptyPerms = (): PermissionState =>
  MODULES.reduce((acc, m) => {
    acc[m] = { view: false, create: false, edit: false, approve: false };
    return acc;
  }, {} as PermissionState);
...
export function NewRoleModal({ open, onOpenChange }: Props) {
  const [step, setStep] = useState<Step>("form");
  const [name, setName] = useState("");
  const [department, setDepartment] = useState("");
  const [deptOpen, setDeptOpen] = useState(false);
  const [perms, setPerms] = useState<PermissionState>(emptyPerms());
  ...
  const anyPerm = useMemo(
    () => Object.values(perms).some((row) => Object.values(row).some(Boolean)),
    [perms],
  );

  const canSubmit = Boolean(name && department && anyPerm);
  ...
  const handleConfirm = async () => {
    ...
    try {
      ...
      await upsertRoleFn({
        data: { role: name, department, permissions },
      });
