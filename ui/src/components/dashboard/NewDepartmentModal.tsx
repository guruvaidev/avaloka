...
import { ChevronDown, CheckCircle2, X, HelpCircle, Check } from "lucide-react";
...
const EXISTING_DEPARTMENTS = new Set(["Data Research", "Data Engineering"]);
...
export function NewDepartmentModal({ open, onOpenChange, onAdded }: Props) {
  ...
  const canSubmit = Boolean(name && code);

  const handleSelectDept = (d: string) => {
    setName(d);
    setCode(DEPT_CODE_MAP[d] ?? "");
  ...
  const handleConfirm = async () => {
    ...
    try {
      ...
      toast.error(e instanceof Error ? e.message : "Failed to add division");
      setSubmitting(false);
    }
  };
            ...
            <div className="space-y-4 px-5 py-5">
                ...
                <Popover open={deptOpen} onOpenChange={setDeptOpen}>
                  ...
                  <PopoverContent className="w-[var(--radix-popover-trigger-width)] p-1" align="start">
                    {DEPARTMENT_OPTIONS.map((d) => (
                      ...
                      >
                        {d}
                        {name === d && <Check className="h-4 w-4 text-primary" />}
                      </button>
                    ))}
                  </PopoverContent>
                </Popover>
              ...
              <div className="space-y-1.5">
                <label className="text-sm font-medium text-foreground">
                  Division Head <span className="text-muted-foreground font-normal">(Optional)</span>
                </label>
