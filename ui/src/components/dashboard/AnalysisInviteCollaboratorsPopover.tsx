// Analysis-specific Invite Collaborators modal.
// Mirrors the report share modal (ReportInviteCollaboratorsPopover) exactly,
// but every resource reference points at the ANALYSIS resource:
// - share links use resource_type "analysis" (=> /shared/:token viewer)
// - email sends go to /api/public/send-analysis-email with analysis_id
...
interface AnalysisInviteCollaboratorsPopoverProps {
  open: boolean;
  onClose: () => void;
  analysisId: string;
  className?: string;
...
export function AnalysisInviteCollaboratorsPopover({
  open,
  onClose,
  analysisId,
  className,
  onCollaborationChange,
}: AnalysisInviteCollaboratorsPopoverProps) {
  ...
  const handleSendEmailAnalysis = async () => {
    ...
    try {
      ...
      const data = (await response.json().catch(() => null)) as
        | { ok?: boolean; error?: string; stage?: string; server_reply?: string }
        | null;
      ...
      const d = (err as any)?.detail as { stage?: string; server_reply?: string } | undefined;
      const description = d
        ? [d.stage ? `stage: ${d.stage}` : null, d.server_reply ? `reply: ${d.server_reply.slice(0, 600)}` : null]
            .filter(Boolean)
            .join("\n")
        : undefined;
  ...
  const loadCollaborators = useCallback(async () => {
    ...
    try {
      const [users, teamsRes, membersRes, rows, existingLink] = await Promise.all([
        ...
        loadResourceCollaboratorRows("analysis", analysisId),
        supabase
          .from("resource_share_links")
          .select("share_token,is_public")
          .eq("resource_type", "analysis")
          .eq("resource_id", analysisId)
          .maybeSingle(),
      ]);
      ...
      console.error("[AnalysisInviteCollaboratorsPopover] load failed", err);
      toast.error("Failed to load share settings");
    } finally {
      setConfigLoading(false);
    }
  }, [listAppUsersFn, analysisId]);
  ...
  const shareUrl = shareToken ? shareLinkFullUrl(shareToken, "analysis") : "";

  const handleGenerateLink = async () => {
    setLinkBusy(true);
    try {
      const link = await ensureResourceShareLink("analysis", analysisId);
      const wantPublic = shareVisibility === "Public";
      if (Boolean(link.isPublic) !== wantPublic) {
        await updateResourceShareLinkPublic("analysis", analysisId, wantPublic);
      }
      setShareToken(link.token);
      onCollaborationChange?.();
      toast.success("Share link ready");
    } catch (err) {
      console.error("[AnalysisInviteCollaboratorsPopover] generate link failed", err);
  ...
  const handleVisibilityChange = async (value: string) => {
    ...
    try {
      await updateResourceShareLinkPublic("analysis", analysisId, next === "Public");
      onCollaborationChange?.();
      toast.success(next === "Public" ? "Anyone with the link can view" : "Link now requires sign-in");
    } catch (err) {
      console.error("[AnalysisInviteCollaboratorsPopover] visibility update failed", err);
  ...
  const handleMemberAccessChange = async (memberId: string, value: string) => {
    ...
    try {
      ...
      console.error("[AnalysisInviteCollaboratorsPopover] member access update failed", err);
  ...
  const handleSendInvites = async () => {
    ...
    try {
      ...
      console.error("[AnalysisInviteCollaboratorsPopover] send invites failed", err);
          ...
          {shareToken ? (
            ...
            <>
              ...
              <p className="mt-2 text-xs text-tertiary">
                Choose visibility, then click Generate to create a shareable link to this analysis.
              </p>
            </>
          )}

          {shareToken && linkCopied ? <p className="mt-1 text-xs text-success-primary">Copied!</p> : null}
        </div>

        <div className="mt-5 border-t border-secondary px-5 pt-4">
          <p className="text-sm font-semibold text-primary">Email Analysis</p>
          <p className="mt-1 text-xs text-tertiary">
            Recipients receive a password-protected PDF of this analysis. Share the password separately.
          </p>
          <div className="mt-3 space-y-3">
            <div>
              <label htmlFor="aic-email-recipients" className="text-xs font-medium text-secondary">
                Recipient email(s)
              </label>
              <textarea
                id="aic-email-recipients"
            ...
            <div>
              <label htmlFor="aic-email-password" className="text-xs font-medium text-secondary">
                PDF password
              </label>
              <div className="mt-1 flex items-center gap-2">
                <input
                  id="aic-email-password"
            ...
            <div>
              <label htmlFor="aic-email-message" className="text-xs font-medium text-secondary">
                Message <span className="text-tertiary">(optional)</span>
              </label>
              <textarea
                id="aic-email-message"
                value={emailMessage}
