...
/**
 * Notifications popover scoped to a single project.
 * Shows notifications whose resource is the project itself, or an analysis
 * belonging to the project.
 */
export function ProjectNotificationsPopover({ projectId }: { projectId: string | null }) {
  const { items, markOne, markAll, removeOne, clearAll } = useNotifications();
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      const { data } = await supabase.from("analyses").select("id").eq("project_id", projectId);
      if (cancelled) return;
      setAnalysisIds(((data ?? []) as Array<{ id: string }>).map((r) => r.id));
    })();
    return () => {
      cancelled = true;
    };
  ...
  const scoped = useMemo(() => {
    if (!projectId) return [] as NotificationRow[];
    const aset = new Set(analysisIds);
    return items.filter((n) => {
      if (n.resource_type === "project" && n.resource_id === projectId) return true;
      if (n.resource_type === "analysis" && n.resource_id && aset.has(n.resource_id)) return true;
      return false;
    });
  }, [items, analysisIds, projectId]);
  ...
  useEffect(() => {
    ...
    const ids = Array.from(
      new Set(scoped.filter((n) => n.actor_id && USER_INTERACTION_TYPES.has(n.type)).map((n) => n.actor_id as string)),
  ...
  const markAllScoped = async () => {
    await Promise.all(scoped.filter((n) => !n.read_at).map((n) => markOne(n.id)));
    // Fallback: also call global mark-all if nothing scoped-specific happened.
    if (scoped.length === 0) await markAll();
  };

  const clearAllScoped = async () => {
    if (scoped.length === 0) {
      await clearAll();
      return;
    }
    await Promise.all(scoped.map((n) => removeOne(n.id)));
  };
  ...
  return (
    <Popover>
      <PopoverTrigger asChild>
        <button
          type="button"
          className="relative inline-flex items-center gap-2 rounded-lg border border-secondary bg-primary px-3 py-2 text-sm font-semibold text-primary shadow-xs hover:bg-primary_hover data-[state=open]:border-[#1565ef] data-[state=open]:text-[#1565ef]"
        >
          <Bell01 className="size-4 text-fg-secondary" />
          Notifications
          {unreadCount > 0 && (
            <span className="ml-0.5 inline-flex h-4 min-w-4 items-center justify-center rounded-full bg-red-500 px-1 text-[10px] font-semibold leading-none text-white">
      ...
      <PopoverContent align="end" side="bottom" sideOffset={8} className="w-[360px] p-0">
        <div className="flex items-center justify-between border-b border-border px-4 py-3">
          <div className="text-sm font-semibold text-foreground">Project notifications</div>
          <div className="flex items-center gap-3">
            {unreadCount > 0 && (
              <button type="button" onClick={markAllScoped} className="text-xs font-medium text-primary hover:underline">
                Mark all as read
              </button>
            )}
            {scoped.length > 0 && (
        ...
        <div className="max-h-[440px] overflow-y-auto">
          {scoped.length === 0 ? (
            <div className="flex flex-col items-center justify-center gap-2 px-4 py-10 text-center">
              ...
              <p className="text-sm text-muted-foreground">No notifications for this project yet.</p>
            </div>
          ) : (
                  ...
                  <li key={n.id} className="group relative">
                    ...
                    >
                      ...
                      <div className="min-w-0 flex-1">
                        ...
                        <div className="mt-1 text-[11px] text-muted-foreground">{relTime(n.created_at)}</div>
                      </div>
                    </button>
                    <button
                      type="button"
