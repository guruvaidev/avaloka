...
from app.agents.mta_v2.schema import ModelDetail, ModelSummary
...
class MLflowManager:
    ...
    def get_model_details(self, mlflow_run_id: str) -> Optional[ModelDetail]:
        ...
        metrics = {
            ...
            "final_train_mae":    metrics_raw.get("final_train_mae",    artifact_metrics.get("final_train_mae")),
            "final_mae":          metrics_raw.get("final_mae",          artifact_metrics.get("final_mae")),
            "final_train_rmse":   metrics_raw.get("final_train_rmse",   artifact_metrics.get("final_train_rmse")),
            "final_rmse":         metrics_raw.get("final_rmse",         artifact_metrics.get("final_rmse")),
            "final_train_r2_score": metrics_raw.get("final_train_r2_score", artifact_metrics.get("final_train_r2_score")),
            "final_r2_score":     metrics_raw.get("final_r2_score",     artifact_metrics.get("final_r2_score")),
        ...
        return ModelDetail(
            ...
            inference_service_details=cfg_dict.get("inference_service_details", None)
        )
    ...
    def upload_onnx_model(self, mlflow_run_id: str, model_path: str) -> None:
        """Upload or replace the canonical ONNX artifact for an existing run."""
        ...
        logger.info(
            "Uploaded self-contained ONNX model for run '%s' → %s",
            mlflow_run_id,
            self.ONNX_ARTIFACT_PATH,
        )
    ...
    def load_model(
        ...
        if paths["config"] and Path(paths["config"]).exists():
            ...
            cfg = ModelConfig(
                ...
                mlflow_run_id=mlflow_run_id,
                inference_service_details=detail["inference_service_details"]
            )
