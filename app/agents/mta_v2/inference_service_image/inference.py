...
from schema import ModelDetail, ModelSummary, RayConfig
...
# This module ships INSIDE the inference image (flat imports, `COPY . .`), so it
# cannot import app.core.cloud_config. It applies the same rule by hand: no
# private default for identity — an unset project raises here, naming the
# variable, rather than silently addressing the project this was built in.
def _require_env(name: str, why: str) -> str:
    value = (os.environ.get(name) or "").strip()
    if not value:
        raise RuntimeError(
            f"{name} is not set. {why} Avaloka does not assume a project or "
            f"registry — an assumed one addresses somebody else's account."
        )
    return value


def _ray_gcp_project_id() -> str:
    return _require_env("GOOGLE_CLOUD_PROJECT", "It identifies the GCP project to deploy into.")


def _ray_inference_docker_uri() -> str:
    explicit = (os.environ.get("RAY_INFERENCE_DOCKER_URI") or "").strip()
    if explicit:
        return explicit
    registry = (os.environ.get("CONTAINER_REGISTRY") or "").strip()
    if not registry:
        registry = f"gcr.io/{_ray_gcp_project_id()}"
    return f"{registry.rstrip('/')}/inference-docker-image:test-v0.0.2"
...
class InferenceInterface:
    ...
    def _build_input_tensor(
        ...
        values: List[float] = []
        for feat in cfg.feature_names:
            values.append(self._encode_feature_value(feat, row.get(feat), cfg))
    ...
    def inference_local(
        ...
        input_tensor = torch.tensor(
            prepared_df[cfg.feature_names].values,
            dtype=torch.float32,
            device=self._device,
        )
        ...
        if not gcs_bucket:
            gcs_bucket = dataset_uri.replace("gs://", "").split("/")[0]
    ...
    def inference_on_ray(
        ...
        from gcp.get_k8s_api import get_k8s_api
        from gcp.namespace import create_namespace
        from gcp.create_ray_cluster import create_ray_cluster
        from gcp.get_ray_head_svc import get_ray_head_svc
        from gcp.get_ray_dashboard_url import get_ray_dashboard_url
        from gcp.wait_for_ray_job_completion import wait_for_ray_job_completion
        from gcp.submit_ray_job import submit_ray_job
        from gcp.get_job_result import get_job_result
        from gcp.clean_up import delete_ray_cluster

        # ── 0. Read credentials early so we fail fast ──────────────────────
        _GCP_SA_JSON_PATH = os.getenv("GOOGLE_APPLICATION_CREDENTIALS", "/tmp/gcp_sa.json")
